...
def atLeastOne(literals: List[Expr]) -> Expr:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    return disjoin(
        literals)  # for example if literals = [A,B,C], the A | B | C sentence is True if at least one expression is True
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def atMostOne(literals: List[Expr]) -> Expr:
    ...
    combinations = list(
        itertools.combinations(literals, 2))  # we store all the possible combinations of 2 items from the literals list
    sentences = []
...
def localization(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for (x, y) in non_outer_wall_coords:
            if findModel(
                    conjoin([conjoin(KB), PropSymbolExpr(pacman_str, x, y, time=t)])):  # a satisfying assignment exists
                possible_locations.append((x, y))
            if entails(conjoin(KB), PropSymbolExpr(pacman_str, x, y,
                                                   time=t)) == True:  # locations that pacman is provably at at time = t
                KB.append(PropSymbolExpr(pacman_str, x, y, time=t))
            elif entails(conjoin(KB), ~PropSymbolExpr(pacman_str, x, y,
                                                      time=t)) == True:  # locations that pacman is not provably at at time = t
                KB.append(~PropSymbolExpr(pacman_str, x, y, time=t))
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        KB.append(pacphysicsAxioms(t, all_coords, non_outer_wall_coords, walls_grid=known_map, sensorModel=sensorAxioms,
                                   successorAxioms=allLegalSuccessorAxioms))
        KB.append(PropSymbolExpr(agent.actions[t], time=t))
        percepts = agent.getPercepts()
        result = fourBitPerceptRules(t=t, percepts=percepts)
